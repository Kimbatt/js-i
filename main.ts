...
const textarea = <HTMLTextAreaElement>document.getElementById("text-area");
const canvas = <HTMLCanvasElement>document.getElementById("result-canvas");

const resultTextArea = <HTMLTextAreaElement>document.getElementById("result-text-area");
const imagePathInput = <HTMLInputElement>document.getElementById("image-path-input");
...
function GetRngSeed()
{
    const seedStr = (<HTMLInputElement>document.getElementById("rng-seed")).value;
    if (seedStr === "")
...
function LoadScriptFromImage(imgSrc: string, seed: number)
{
    const img = new Image();
    img.onload = function()
    {
        ...
        const rand = (function(seed: number)
        {
            const imul = Math.imul;
            return function()
            {
        ...
        const resultBytes = new Array<number>(dataLength);
...
function GetLoadMinifiedString(seed: number, imageSrc: string)
{
...
function ModeChanged(elem: HTMLSelectElement)
{
...
let loadedImageData: Uint8ClampedArray | null = null;
function FileSelected(elem: HTMLInputElement)
{
...
let imageBitcount = 1;
function ImageBitcountChanged(bitCount: number)
{
    imageBitcount = bitCount;
    bitCountText.textContent = bitCount.toString();
}

function GenerateAndHide(inputBytes: number[], headerBytes: number[], rand: () => number, seed: number)
{
    if (!loadedImageData)
    {
    ...
    const shouldFillImageWithRandomData = (<HTMLInputElement>document.getElementById("fill-image-with-random-data-checkbox")).checked;
...
function LoadFromHiddenImage(imgSrc: string, seed: number)
{
    const img = new Image();
    img.onload = function()
    {
...
function GetLoadHiddenMinifiedString(imgSrc: string, seed: number)
{
